...
#explaination: use two variations to solve the problem of the routing table to make it solvable raw data
#no matter the raw data or the correct BGP DUMP DATA would be accepted by the system without any extra requirement
...
die "Cannot open file $tempinfile" if ( ($tempinfile) && !( open (INPUT,"<$tempinfile") ) );
...
if (<INPUT>)
{
	...
	{
		$infile = $tempinfile;
		sub_debug("m","this is the if case first location.\n");
	}
	else
	{
		system("zcat $tempinfile | ./bgpdump -m - > myroutes" );
		$infile = "./myroutes";
		sub_debug("m","this is the if case second location.\n");

	}
}
else
{
	sub_debug("m","=======================this is the locattion that do not want to be appear! \n");
}
