...
die "Peer IP address is not valid: $peerip2"     if (sub_checkip($peerip2));#==============================================
die "Peer AS number is not valid: $peeras2"      if (sub_checkas($peeras2));#==============================================
...
sub_debug ("m", "Configured for an $peer_type session between me (ASN$myas, $myip) and peer (ASN$peeras2, $peerip2).\n");
...
sub_debug("m","this is the test location one \n");
$peer->add_timer(\&sub_timer_callback, 10);
#there must be one timer to use ashnolization ,to make peers in this timer.
sub_debug("m","this is the test location two \n");

$bgp->add_peer($peer); #to make AS and ip be recognized by host ,it must be added to

#$peer2->add_timer(\&sub_timer_callback, 10);
sub_debug("m","this is the test location three \n");

#$peer2->add_timer(\&sub_timer_callback, 10);

#$bgp->add_peer($peer); #to make AS and ip be recognized by host ,it must be added to
sub_debug("m","this is the test location four \n");

$bgp->add_peer($peer2);
sub_debug("m","this is the test location five \n");

$peer->start();
sub_debug("m","this is the test location six \n");

$peer2->start();
sub_debug("m","this is the test location seven \n");

$bgp->event_loop();
sub_debug("m","this is the test location eight \n");
...
#	my ($peer3)=shift(@_);
        my $peerid = $peer->peer_id();
        my $peeras = $peer->peer_as();
#	my $peerid2= $peer3->peer_id();
	sub_debug ("m","send update to peer $peerid\n");


	if (! $peer->is_established)
	{ 
		sub_debug ("m","send update to peer $peerid(111111111111111)\n");

		sub_debug ("d", "Loop: trying to establish session.\n");
		sub_connect_peer($peer);
		sub_connect_peer($peer2); 

	} elsif (($infile) && (! $full_update))
	{ 
                sub_debug ("m","send update to peer $peerid(2222222222222222222) with full update\n");
	
		sub_debug ("m","Sending full update.\n");

		$full_update = 1;
#               sub_debug ("m","peer 1 ip address $peerid is $peer->peer_id() \n");
#		sub_debug ("m","peer 2 ip address $peerid2  is $peer3->peer_id() \n");
		sub_update_from_file($peer,$peer2);

		sub_debug ("m", "Full update sent.\n");
	} else
	{
                sub_debug ("m","send update to peer $peerid(333333333333333333) with nothing to do \n");
