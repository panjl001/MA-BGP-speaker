...
#			print "bgp file session file is : $ip_peer , bgp routing information is : $route , filter file is : $filter \n";
 	...
 	#			print "$ip_peer      $route";
 				...
 				if ((!open(IPFILE,$ip_peer))|(!open(ROUTEFILE,$route)))
				{
 				...
 				{
 					  print "debug location 333333333333333333333\n";
                                        sleep 5;
                                        if (<ROUTEFILE>)
                                        {
                                                print "debug location 44444444444444444444444\n";
                                                sleep 5;
                                                if (<ROUTEFILE> =~ /TABLE/)
                                                {
                                                         rename($route,"routing_information_base");
                                                        print "debug location 1111111111111111\n";
                                                        sleep 5;
                                                }
                                                else
                                                {
                                                        system("zcat $route | ./bgpdump -m - > routing_information_base" );
                                                #        $infile = "./myroutes";
                                                #       sub_debug("m","this is the if case second location.\n");
                                                        print "debug location 222222222222222222\n";
                                                        sleep 5;
