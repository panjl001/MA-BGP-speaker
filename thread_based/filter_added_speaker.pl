...
# 					print "initalizing BGP program\n";
 					...
 					if ($filter)
 					{
						...
						close FILTERFILE;
						rename($filter,"filter_information_base");
...
sub sub_update_from_file
{
	...
	my ($peer) = shift(@_);
	my $infile = "routing_information_base";
	my $filter_name = "filter_information_base";
...
#	sleep 10;
	...
	if ($filter_name)
	{
			...
			{
				my $getin = $_;
				my @getip = split /\@/, $getin;

				      chomp ($getip[1]);
#                                                print "this is a debug location to show sth: $getip[0],\n  this is to show ip address : $getip[1]\n";





			#	print "this is to show inital info: $getip[0]    and $getip[1]  \n";

				if ($getip[1] eq $temple)
				{
					my @temp =  split /\=/,$getip[0];
				#	print "@temp \n";
					%regex_filter = ( $temp[0] => $temp[1] );
				...
				#	print "this is mark location 1111111!\n";		
					goto finishmatching;
				}
				else
				{
			...
			finishmatching:
			foreach (keys %regex_filter)
			{
				print "$_ \n";
				print "$regex_filter{$_}\n";
			};
			sleep 3;
			close FILTER;
	}
...
#			print "this is used to test a debug location to show information\n this is next hop : $nexthop \n this is aspath:$aspath\n";;;;;
		#	print "this is mark location2222222222222222!\n ";
		#	sleep 3;
			next;
		};
